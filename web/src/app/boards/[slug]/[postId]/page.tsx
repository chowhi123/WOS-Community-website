...
export default function BoardPostPage() {
    const params = useParams(); // { slug, postId }
    ...
    return (
        <div className="min-h-screen p-4 md:p-8">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                {/* Back Link - Use slug from params or post.board */}
                <Link href={`/boards/${slug}`} className="text-slate-500 hover:text-ice-400 text-sm transition-colors mb-2 inline-block">
                        ...
                        {session?.user && (
                            <div className="mb-4 flex gap-2 justify-end">
                                {((session.user as { id?: string }).id === post.author.id) && (
                                    <Link
                                        href={`/boards/${slug}/${postId}/edit`} // Adjusted for new structure
